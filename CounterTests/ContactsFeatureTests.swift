...
struct ContactsFeatureTests {
    ...
    func addFlow() async {
        ...
        await store.send(\.destination.addContact.setName, "Luu Nguyen") {
            $0.destination?.modify(\.addContact) { $0.contact.name = "Luu Nguyen" }
        }

        await store.send(\.destination.addContact.saveButtonTapped)
        await store.receive(\.destination.addContact.delegate.saveContact,
                            Contact(id: UUID(0), name: "Luu Nguyen"))
        {
            $0.contacts = [Contact(id: UUID(0), name: "Luu Nguyen")]
        }
    ...
    func deleteContact() async {
        let store = TestStore(initialState:
            ContactsFeature.State(contacts: [
